...
**If a `coding-session` dispatched you as its review worker, stop after posting the review.** Post per-finding comments (file:line, severity) plus one summary verdict comment on the PR and return the findings to the conductor — it owns the fix loop, its round budget, and the CI re-check (`${CLAUDE_PLUGIN_ROOT}/shared/ci-convergence.md`). Two threads driving the same loop double-push and blow the bounds.
