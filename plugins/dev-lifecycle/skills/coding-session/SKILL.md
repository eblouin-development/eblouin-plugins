...
- **Every subagent gets the canon, and proves it took it.** A brief names the plugin skill in invocable form (`dev-lifecycle:backend`), the shared doctrines by path, and the repo's own `CLAUDE.md` and project-local skills — and the worker opens its report with what it loaded. Without that, a subagent does the work from general knowledge, and the result reads fine while quietly ignoring the conventions this library exists to enforce (see "Subagent briefing notes").
...
- **The continuation brief is a sticky comment on the draft PR**, refreshed in place at every step boundary — issue/PR/branch, per-track status, live workers with their task IDs and watchdogs, the CI-green/review-clean loop state and its round counters, decisions since the last refresh, open findings, checkpoints passed and ahead, the environment facts a resuming session would otherwise rediscover the hard way (toolchain versions, services to start by hand, a gate that can't run concurrently), and the single next action. Keeping it current is a few edited lines; it's what makes the handoff nearly free.
...
Brief the subagent with: the issue number, this step's slice of the plan and its acceptance criteria, the **feature branch** name, and — always — the **canon block** below. A subagent that doesn't load the firm's skills will do the work from its own general knowledge and produce something plausible that ignores every convention this library encodes; naming a skill in prose is not the same as the subagent loading it.
...
- **Open its report with the canon it loaded** — the plugin skill(s), the shared doctrines, and whether the repo had a `CLAUDE.md` and project-local skills (and if so, the conventions it took from them). This one line is what makes the canon block verifiable instead of aspirational: a worker that silently skipped it is otherwise undetectable, and its output looks fine right up until review.
- Report back what it built, the commits it pushed, and anything it couldn't resolve.

**If a returning worker's report doesn't name the skills, treat the step as unverified against canon** — don't accept it. Re-dispatch with the canon block restated, or (for a small step) have the review worker check the diff against the skill's standards explicitly. Two silent workers in a row on the same feature is worth noting in the decision log.
...
**7b. Review the green branch, comments on the PR.** Spawn a review subagent on **`opus`**, briefed with the canon block to invoke `dev-lifecycle:code-review` in **pipeline mode** on the full PR diff. This is the *integration* review, and its charter is deliberately different from the per-step passes: it does not re-litigate settled findings, it hunts what per-step review structurally cannot see — cross-step contradictions, comments and assumptions invalidated by a later step, an earlier ruling applied to only some of its sites, the acceptance criteria end to end, and interaction with other in-flight PRs. It **posts its findings on the PR**: per-finding comments with file:line, severity-ranked, plus a single summary comment with the verdict. This is the written review the human's sign-off leans on. Same worker-cadence discipline: background dispatch, its own watchdog.
...
- **Red almost immediately — or after sitting far past the normal queue — is probably not a failure at all.** A job that fails in seconds never ran anything, and the same outage also shows up as a long queue then a fail, so don't judge on duration alone. Gather the evidence (`runner_id: 0`, empty `runner_name`, logs 404, empty check-run output, unrelated jobs failing identically, none of this diff's files in the failing job's scope, reproduces on the base branch), then **ask the user to confirm it's an Actions/provider failure**, showing the evidence rather than just the conclusion. **Don't block on the answer**: return the PR to **draft** and keep converging locally, which is right either way. On confirmation (or no answer), stay in draft until the local gate is green *and* the review is clean, then flip once — saying plainly that the gate was local, which checks ran, and which jobs are unverified. If the user says it isn't Actions, treat it as ours and run the normal fix loop. Full protocol: `${CLAUDE_PLUGIN_ROOT}/shared/ci-convergence.md`.
- **Red, and it isn't ours in some other way** (a rate limit, an expired or missing repo secret, a pre-existing flake, a failure that reproduces identically on the base branch of an otherwise-healthy pipeline) → **leave the PR ready** and notify the user with the diagnosis: which job, why it isn't the code, and what would clear it. Flipping back to draft here would hide a mergeable PR behind a failure the code didn't cause and the session can't fix. The difference from the case above is whether the pipeline can still return a verdict at all: if it can, one explained red check on a ready PR is honest; if it can't, ready would claim a verification that doesn't exist.
...
- **One skill focus per subagent, named in invocable form.** A subagent is briefed to invoke a specific skill on a specific step or PR — written as `dev-lifecycle:<name>` with the instruction to load it via the Skill tool, never as a bare noun the agent can satisfy from its own knowledge. Don't hand a subagent the whole session — hand it one stage of it. Its context dies when it returns; only what it reports (commits pushed, a verdict, a blocker) survives into the conductor.
- **Every brief carries the canon block.** The plugin-qualified skill, the shared doctrines by path, the repo's own `CLAUDE.md` and project-local skills, and the instruction to report what it loaded (step 4). This applies to build workers, fix workers, gate workers and review workers alike — a fix worker that skips canon reintroduces exactly what the review just caught.
...
- Flip a ready PR back to draft on a red run it hasn't diagnosed — diagnosis comes first, and what it finds decides the state.
- Dispatch a fix worker at a job that never ran — no runner, no logs, no output is not a failing test.
- Keep flipping to ready to see whether CI works. When the pipeline can't return a verdict, converge in draft and flip once, when the work is actually done.
- Brief a subagent with a bare skill name and no canon block, or accept a worker's report that doesn't say what it loaded.
