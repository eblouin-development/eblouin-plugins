...
- A **token-efficiency doctrine** every skill follows, a **worker-cadence doctrine** for how an orchestrator watches the subagents it dispatches (backstop cadence, not polling — used by `coding-session`), a **parallelization doctrine** that has a session work out the dependency graph and run independent tracks concurrently by default (in isolated worktrees), and a shared **definition-of-done** that is the merge-ready bar.
