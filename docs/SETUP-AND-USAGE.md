...
- A **token-efficiency doctrine** every skill follows, a **worker-cadence doctrine** for how an orchestrator watches the subagents it dispatches (backstop cadence, not polling — used by `coding-session`), a **parallelization doctrine** that has a session work out the dependency graph and run independent tracks concurrently by default (in isolated worktrees), a **CI-convergence doctrine** where the session owns the push → CI → fix → review → fix loop to green (watchdogged on both halves, and run locally in the container if CI is down), a **context-continuity doctrine** that has a long session hand off to itself at ~75% of its window — flush state to the issue/PR, refresh a continuation brief, reload the skills in play, resume without involving you — an **issue-cross-check doctrine** that runs every generated plan against the open backlog (what it closes, duplicates, or conflicts with) before you're asked to approve it, and a shared **definition-of-done** that is the merge-ready bar.
